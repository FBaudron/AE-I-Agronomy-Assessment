...
data = read.xlsx("ClimateEngine_Mbire.xlsx", sheet = 1)
...
ggplot(graph, aes(color = year, fill = year)) + ggtitle("Mbire - Zimbabwe") +
...
# ggsave("Climate Mbire - Zimbabwe.jpeg", units = "cm", width = 30, height = 20, dpi = 320)


# MAKUENI ----------------------------------------------------------------------

data = read.xlsx("ClimateEngine_Murehwa.xlsx", sheet = 1)
...
ggplot(graph, aes(color = year, fill = year)) + ggtitle("Murehwa - Zimbabwe") +
  geom_bar(mapping = aes(x = month, y = precipitation * max(na.omit(temperature))/max(precipitation)),
           stat = "identity", position = "dodge", alpha = 0.7) +
  geom_point(mapping = aes(x = month, y = temperature), size = 4, alpha = 0.7) + 
  geom_line(mapping = aes(x = month, y = temperature), size = 1) + 
...
# ggsave("Climate Murehwa - Zimbabwe.jpeg", units = "cm", width = 30, height = 20, dpi = 320)
